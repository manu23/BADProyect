...
public class PrerrequisitoConverter implements Converter {
    ...
    entities.PrerrequisitoPK getKey(String value) {
        entities.PrerrequisitoPK key;
        String values[] = value.split(SEPARATOR_ESCAPED);
        key = new entities.PrerrequisitoPK();
        key.setIdpropuesta(values[0]);
        key.setCodigomateria(values[1]);
        return key;
    }

    String getStringKey(entities.PrerrequisitoPK value) {
